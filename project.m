...
for currElemNumber = 1:numElements
    ...
    currElemCoords = nodes(elements(currElemNumber, :), :);
    ...
    for gaussPoint = 1:4
        ...
        zeta = gaussTable(gaussPoint, 1);    
        eta = gaussTable(gaussPoint, 2);
...
function [J, detJ] = jacobian(coords, dN_dzeta, dN_deta)
    ...
    x}{\partial zeta} = \Sum_{i=1}^{M}x_i * \frac{\partial N_i(zeta, eta}}{\partial zeta}. We get
